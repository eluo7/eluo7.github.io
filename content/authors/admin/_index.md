...
awards:
  ...
  - title: 客户信用评级模型
    ...
    summary: |
      - 模型架构设计  
        - 客户分层子模型矩阵：基于客户属性、交易行为及风险特征的差异化，构建精细化分层体系，设计多场景准入 / 复评子模型，实现客群定制化风险评估。
        - 教师 - 学生双模型框架：借鉴 FICO 框架思想，以 LightGBM/XGBoost 集成学习模型作为 “教师” 捕捉非线性风险模式，逻辑回归作为 “学生” 构建可解释评分卡，通过概率输出校准实现预测能力与业务可解释性的平衡。
      
      - 算法选型与优化  
        - 对比逻辑回归、集成学习等算法，选定 LightGBM/XGBoost 集成学习算法，在保证预测精度的同时满足可解释性要求。
        - 引入 AutoML 自动化流程，完成特征工程（如主成分分析降维、互信息特征筛选）与超参数调优，提升模型开发效率 30%+。
        - 运用 SHAP 可解释 AI 技术解析模型决策逻辑，量化特征贡献度，优化特征权重分配；基于教师模型预测概率，采用等频分箱或决策树递归分箱，确保分箱内风险分布同质。
      
      - 模型评估与落地  
        - 通过 AUC、PSI 等指标完成模型区分能力、稳定性评估；
        - 上线后监控逾期率、坏账率、回款周期等业务指标，较传统模型实现坏账率降低，资金回收效率提升，同时识别高价值客户贡献度增长，验证模型有效性与业务价值。
...
拥有多年AI和机器学习经验，不仅专注于算法模型构建与大数据处理技术，更擅长深入理解业务需求，通过跨部门协作、项目统筹与持续优化，将技术成果转化为实际业务价值，为企业业务风险防控与发展决策提供全方位支持。